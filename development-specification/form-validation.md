...
|校验项|规则|示例|正则|[最大长度](#最大长度)|
|:----:|:----:|:----:|:----:|:----:|
|用户名| 由 1到64位 的字符（除[特殊字符](#特殊字符)）组成|username_yf-2016|-- |64|
|密码| 由 1到20位 的字符 |2019abcd|--|20|


#### 注册
|校验项|规则|示例|正则|[最大长度](#最大长度)|
|:----:|:----:|:----:|:----:|:----:|
|用户名| 由 6到64位 的字符组成|username_yf-2016|`/^[0-9_a-zA-Z]{6,64}$/`|64|
|用户名（增强验证）|不能为纯数字|123456|`/^([+-]?)\d*\.?\d+$/`|64|
|密码| 由 6到20位 的字符 |2019abcd|`/^[\x00-\xFF]{6,20}$/`|20|
...
|:----:|:----:|:----:|:----:|:----:|
|邮箱|由 用户名 + '@' + 域名 组成|`username@xxx.com`|`/^([a-zA-Z0-9])+([a-zA-Z0-9_\.\-])*@[A-Za-z0-9]+([-.][A-Za-z0-9]+)*\.[A-Za-z0-9]+([-.][A-Za-z0-9]+)*$/`|--|
...
|银行卡| 一般由 16 或 19位的数字组成，卡号规则[\[1\]](#[1])，校验列表[\[2\]](#[2]) | -- |`/^[1-9](\d{15}\|\d{18})$/`|19|
...
#### 特殊字符   
说明：英文字符下的 单双引号、分号、两个连续的连字符（' " ; -- ）  
正则： `/.*('|;|\"|--).*/`


#### 最大长度
input 输入框的 maxlength 属性，注意只有 type 是 text, search, url, tel, email, password 的 input 支持此属性      
例如：
...
#### [2] 
国内各个银行的卡号规则   https://github.com/navyxie/bankcardinfo/blob/master/index.js#L40